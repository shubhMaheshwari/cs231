...
def discriminator_loss(logits_real, logits_fake):
    ...
    N = logits_real.data.size()[0]
    
    true_labes = Variable(torch.ones_like(logits_real.data))
    fake_labes = Variable(torch.zeros_like(logits_real.data))
    
    loss = bce_loss(logits_real,true_labes) + bce_loss(logits_fake,fake_labes)
    return loss
...
def generator_loss(logits_fake):
    ...
    """
    true_labes = Variable(torch.ones_like(logits_fake.data)).type(dtype)
    loss = bce_loss(logits_fake,true_labes)
    return loss
